...
// tworzenie nagrobków  ( Graves)

const graves = new THREE.Group();
scene.add(graves);

const grabeGeometri = new THREE.BoxGeometry(0.5, 0.8, 0.2);
const grabMaterial = new THREE.MeshStandardMaterial({ color: "#b2b6b1" });
...
for (let i = 0; i < 50; i++) {
  ...
  const grabe = new THREE.Mesh(grabeGeometri, grabMaterial);
  grabe.position.set(x, 0.3, z);
  grabe.rotation.y = (Math.random() - 0.5) * 0.4;
  grabe.rotation.z = (Math.random() - 0.5) * 0.4;
  grabe.castShadow = true;
  //   grabe.position.set(x, 0, z);
  graves.add(grabe);
}
